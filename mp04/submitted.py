...
class NeuralNet(torch.nn.Module):
    def __init__(self):
        ...
        self.hidden = torch.nn.Sequential(
            torch.nn.Linear(3*31*31, 200),
            torch.nn.ReLU(),
            torch.nn.Linear(200, 164),
            torch.nn.ReLU(),
            torch.nn.Linear(164, 132),
            torch.nn.ReLU(),
            torch.nn.Linear(132, 101),
            torch.nn.ReLU()
        )
        self.output = torch.nn.Sequential(
            torch.nn.Linear(101,64),
            torch.nn.ReLU(),
            torch.nn.Linear(64,23),
            torch.nn.ReLU(),
            torch.nn.Linear(23,5)
...
def train(train_dataloader, model, loss_fn, optimizer):
    ...
    ################# Your Code Starts Here #################
    for features, labels in train_dataloader: # features -> x, labels -> y
